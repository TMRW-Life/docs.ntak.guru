...
This endpoint allows you to retrieve an accommodation by providing the accommodation `id` or `slug`.
...
<Tabs items={['cURL', 'PHP']}>
  <Tabs.Tab>
    [//]: # (@formatter:off)
    ```bash copy
    curl -G https://api.sandbox.ntak.guru/v1/accommodations/rqdsw \
